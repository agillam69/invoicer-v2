...
def test_full_invoice_status_table() -> None:
    ...
    assert derive_status(total_cents=100, due_date=date(2026, 6, 26), today=TODAY) == InvoiceStatus.ISSUED
    assert derive_status(total_cents=100, payment_cents=25, due_date=date(2026, 6, 26),
                         today=TODAY) == InvoiceStatus.PART_PAID
    assert derive_status(total_cents=100, payment_cents=100, today=TODAY) == InvoiceStatus.PAID
    assert derive_status(total_cents=100, due_date=date(2026, 6, 24), today=TODAY) == InvoiceStatus.OVERDUE
