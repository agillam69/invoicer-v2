...
class Invoice(Base):
    ...
    __table_args__ = (
        CheckConstraint("status_override IS NULL OR status_override IN ('Draft','Cancelled','Void')",
                        name="ck_invoice_override"),
        CheckConstraint("subtotal_cents >= 0 AND gst_cents >= 0 AND total_cents >= 0",
                        name="ck_invoice_money"),
    )
...
class InvoiceItem(Base):
    ...
    service_item_id: Mapped[int | None] = mapped_column(ForeignKey("service_items.id", ondelete="SET NULL"))
    ...
    __table_args__ = (
        CheckConstraint("quantity_decimal > 0 AND unit_price_cents >= 0", name="ck_item_quantity_price"),
        CheckConstraint("discount_cents >= 0 AND subtotal_cents >= 0 AND gst_cents >= 0 AND total_cents >= 0",
                        name="ck_item_money"),
    )
...
class LedgerEntry(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("ex_gst_cents >= 0 AND gst_cents >= 0 AND total_cents >= 0",
                        name="ck_ledger_money"),
    )
...
class MigrationRun(Base):
    ...
    report_document_id: Mapped[int | None] = mapped_column(ForeignKey("documents.id", ondelete="SET NULL"))
...
for _table in (Invoice.__table__, InvoiceItem.__table__, Payment.__table__, Receipt.__table__):
    Index(f"ix_{_table.name}_created_at", _table.c.created_at)
